...
public class QuantityMeasurementApp
{
    ...
    public class QuantityWeight : IEquatable<QuantityWeight>
    {
        ...
        public static double GramToKilogramConversionFactorValue => WeightUnit.Gram.GetConversionFactor();
        public static double PoundToKilogramConversionFactorValue => WeightUnit.Pound.GetConversionFactor();
        ...
        private static double ConvertToBase(double value,WeightUnit unit)
        {
            return unit.ConvertToBaseUnit(value);
        }

        private static double ConvertFromBase(double baseValue,WeightUnit unit)
        {
            return unit.ConvertFromBaseUnit(baseValue);
        }

        private double ConvertUnitsToKilogram()
        {
            return _unit.ConvertToBaseUnit(_value);
        }
        ...
        {
            double valueInKilogram = ConvertUnitsToKilogram();
        ...
        public override bool Equals(object obj)
        {
            if (obj is null || this.GetType() != obj.GetType())
                return false;
            return Equals((QuantityWeight)obj);
        }
        ...
        public bool Equals(QuantityWeight other)
        {
            if (other is null)
                return false;
            double thisValueInKilogram = this.ConvertUnitsToKilogram();
            double otherValueInKilogram = other.ConvertUnitsToKilogram();
            return Math.Abs(thisValueInKilogram - otherValueInKilogram) < Tolerance;
        }

        public override int GetHashCode()
        {
            double valueInKilogram = ConvertUnitsToKilogram();
            double normalized = Math.Round(valueInKilogram,4);
            return normalized.GetHashCode();
        }
        ...
        private static QuantityWeight AddWithTargetUnit(QuantityWeight first,QuantityWeight second,WeightUnit targetUnit)
        {
            double firstInBase = ConvertToBase(first.Value,first.Unit);
            double secondInBase = ConvertToBase(second.Value,second.Unit);
            double sumInBase = firstInBase + secondInBase;
    ...
    static void Main(string[] args)
    {
        ...
        Console.WriteLine("\nWeight Equality & Conversion Demonstration:\n");

        //Basic equality
        var kg1 = new QuantityWeight(1.0,WeightUnit.Kilogram);
        var kg2 = new QuantityWeight(1.0,WeightUnit.Kilogram);
        Console.WriteLine($"kg equality: {kg1} equals {kg2}? {kg1.Equals(kg2)}");

        //Cross unit equality
        var kg = new QuantityWeight(1.0,WeightUnit.Kilogram);
        var grams1000 = new QuantityWeight(1000.0,WeightUnit.Gram);
        Console.WriteLine($"kg to gram equality: {kg} equals {grams1000}? {kg.Equals(grams1000)}");

        //Pound equality
        var pound = new QuantityWeight(2.20462,WeightUnit.Pound);
        Console.WriteLine($"kg to pound equality: {kg} equals {pound}? {kg.Equals(pound)}");

        //Conversion demo
        double kgToGram = kg.ConvertTo(WeightUnit.Gram);
        Console.WriteLine($"convert 1 kg to grams: {kgToGram} g");

        double gramToKg = grams1000.ConvertTo(WeightUnit.Kilogram);
        Console.WriteLine($"convert 1000 g to kg: {gramToKg} kg");

        double kgToPound = kg.ConvertTo(WeightUnit.Pound);
        Console.WriteLine($"convert 1 kg to pounds: {kgToPound:F5} lb");

        //Addition demo
        Console.WriteLine("\nWeight Addition:");

        var addKg = new QuantityWeight(2.0,WeightUnit.Kilogram);
        var addGram = new QuantityWeight(500.0,WeightUnit.Gram);

        var addResultDefault = QuantityWeight.Add(addKg,addGram);
        Console.WriteLine($"add({addKg},{addGram}) = {addResultDefault}");

        var addResultGram = QuantityWeight.Add(addKg,addGram,WeightUnit.Gram);
        Console.WriteLine($"add({addKg},{addGram},target=gram) = {addResultGram}");

        var addResultPound = QuantityWeight.Add(addKg,addGram,WeightUnit.Pound);
        Console.WriteLine($"add({addKg},{addGram},target=pound) = {addResultPound:F5}");

        //Negative value demonstration
        Console.WriteLine("\nNegative Weight Demonstration:");

        var negKg = new QuantityWeight(-1.0,WeightUnit.Kilogram);
        var negGram = new QuantityWeight(-1000.0,WeightUnit.Gram);

        Console.WriteLine($"negative equality: {negKg} equals {negGram}? {negKg.Equals(negGram)}");

        var negAddResult = QuantityWeight.Add(kg,negGram);
        Console.WriteLine($"add({kg},{negGram}) = {negAddResult}");
