...
namespace QuantityManagementApp.Tests
{
    ...
    public class IntegrationTests
    {
        ...
        private ILogger<QuantityMeasurementService> _logger;

        [SetUp]
        public void Setup()
        {
            _repository = new QuantityMeasurementCacheRepository(NullLogger<QuantityMeasurementCacheRepository>.Instance);
            _logger = NullLogger<QuantityMeasurementService>.Instance;
            _service = new QuantityMeasurementService(_repository, _logger);
        }
        ...
        [Test]
        public void TemperatureOperations_ThrowNotSupported()
        {
            // Arrange
            ...
            // Act
            var addResult = _service.AddQuantities(celsius100, celsius50);
            var subtractResult = _service.SubtractQuantities(celsius100, celsius50);
            var divideResult = _service.DivideQuantities(celsius100, celsius50);

            // Assert
            Assert.That(addResult.HasError, Is.True);
            Assert.That(subtractResult.HasError, Is.True);
            Assert.That(divideResult.HasError, Is.True);
            Assert.That(addResult.ErrorMessage, Does.Contain("not support"));
        }
    }
}
