...
public class QuantityMeasurementApp
{
    static void Main(string[] args)
    {
        ...
        //category incompatibility demonstration - RENAMED VARIABLES
        WriteLine("\nCategory Incompatibility (Volume vs Others):");

        var volumeForCompare1 = new Quantity<VolumeUnit>(1.0, VolumeUnit.Litre);
        var lengthForCompare1 = new Quantity<LengthUnit>(1.0, LengthUnit.Feet);
        var weightForCompare1 = new Quantity<WeightUnit>(1.0, WeightUnit.Kilogram);

        WriteLine($"volume (1 L) equals length (1 ft)? {volumeForCompare1.Equals(lengthForCompare1)}");
        WriteLine($"volume (1 L) equals weight (1 kg)? {volumeForCompare1.Equals(weightForCompare1)}");
        ...
        WriteLine("SUBTRACTION OPERATIONS (UC12)");
        

        // Length subtraction demonstrations
        WriteLine("\nLength Subtraction:");

        var length1 = new Quantity<LengthUnit>(10.0, LengthUnit.Feet);
        var length2 = new Quantity<LengthUnit>(6.0, LengthUnit.Inch);
        var lengthSubResult = length1.Subtract(length2);
        WriteLine($"10 ft - 6 in = {lengthSubResult} (implicit target)");

        var lengthSubExplicitInches = length1.Subtract(length2, LengthUnit.Inch);
        WriteLine($"10 ft - 6 in = {lengthSubExplicitInches} (target inches)");

        var lengthSubNegative = new Quantity<LengthUnit>(5.0, LengthUnit.Feet).Subtract(new Quantity<LengthUnit>(10.0, LengthUnit.Feet));
        WriteLine($"5 ft - 10 ft = {lengthSubNegative}");

        var lengthSubZero = new Quantity<LengthUnit>(10.0, LengthUnit.Feet).Subtract(new Quantity<LengthUnit>(120.0, LengthUnit.Inch));
        WriteLine($"10 ft - 120 in = {lengthSubZero}");

        // Weight subtraction demonstrations
        WriteLine("\nWeight Subtraction:");

        var weight1 = new Quantity<WeightUnit>(10.0, WeightUnit.Kilogram);
        var weight2 = new Quantity<WeightUnit>(5000.0, WeightUnit.Gram);
        var weightSubResult = weight1.Subtract(weight2);
        WriteLine($"10 kg - 5000 g = {weightSubResult}");

        var weightSubExplicitGram = weight1.Subtract(weight2, WeightUnit.Gram);
        WriteLine($"10 kg - 5000 g = {weightSubExplicitGram} (target grams)");

        // Volume subtraction demonstrations
        WriteLine("\nVolume Subtraction:");

        var volume1 = new Quantity<VolumeUnit>(5.0, VolumeUnit.Litre);
        var volume2 = new Quantity<VolumeUnit>(500.0, VolumeUnit.Millilitre);
        var volumeSubResult = volume1.Subtract(volume2);
        WriteLine($"5 L - 500 mL = {volumeSubResult}");

        var volumeSubExplicitML = volume1.Subtract(volume2, VolumeUnit.Millilitre);
        WriteLine($"5 L - 500 mL = {volumeSubExplicitML} (target mL)");

        var volumeSubExplicitGal = volume1.Subtract(volume2, VolumeUnit.Gallon);
        WriteLine($"5 L - 500 mL = {volumeSubExplicitGal:F5} (target gallons)");

        
        WriteLine("DIVISION OPERATIONS (UC12)");
        

        // Length division demonstrations
        WriteLine("\nLength Division (dimensionless ratios):");

        var lenDiv1 = new Quantity<LengthUnit>(10.0, LengthUnit.Feet);
        var lenDiv2 = new Quantity<LengthUnit>(2.0, LengthUnit.Feet);
        var lenDivResult = lenDiv1.Divide(lenDiv2);
        WriteLine($"10 ft ÷ 2 ft = {lenDivResult}");

        var lenDivCross = new Quantity<LengthUnit>(24.0, LengthUnit.Inch).Divide(new Quantity<LengthUnit>(2.0, LengthUnit.Feet));
        WriteLine($"24 in ÷ 2 ft = {lenDivCross}");

        var lenDivLessThanOne = new Quantity<LengthUnit>(5.0, LengthUnit.Feet).Divide(new Quantity<LengthUnit>(10.0, LengthUnit.Feet));
        WriteLine($"5 ft ÷ 10 ft = {lenDivLessThanOne}");

        // Weight division demonstrations
        WriteLine("\nWeight Division:");

        var weightDiv1 = new Quantity<WeightUnit>(2000.0, WeightUnit.Gram);
        var weightDiv2 = new Quantity<WeightUnit>(1.0, WeightUnit.Kilogram);
        var weightDivResult = weightDiv1.Divide(weightDiv2);
        WriteLine($"2000 g ÷ 1 kg = {weightDivResult}");

        // Volume division demonstrations
        WriteLine("\nVolume Division:");

        var volumeDiv1 = new Quantity<VolumeUnit>(1000.0, VolumeUnit.Millilitre);
        var volumeDiv2 = new Quantity<VolumeUnit>(1.0, VolumeUnit.Litre);
        var volumeDivResult = volumeDiv1.Divide(volumeDiv2);
        WriteLine($"1000 mL ÷ 1 L = {volumeDivResult}");
        ...
        try
        {
            var divByZero = new Quantity<LengthUnit>(10.0, LengthUnit.Feet).Divide(new Quantity<LengthUnit>(0.0, LengthUnit.Feet));
        }
        catch (DivideByZeroException ex)
        {
            WriteLine($"Division by zero error: {ex.Message}");
        }

        // Cross-category prevention demonstration - RENAMED VARIABLES
        
        WriteLine("CROSS-CATEGORY TYPE SAFETY");
        

        // Demonstrate compile-time type safety
        WriteLine("\nCompile-time Protection:");
        WriteLine("Cannot subtract Weight from Length (compiler error)");
        WriteLine("Cannot divide Volume by Weight (compiler error)");
        WriteLine("Generic type parameter ensures category isolation");
        WriteLine();

        // Demonstrate runtime type checking with Equals - RENAMED VARIABLES
        WriteLine("Runtime Type Checking with Equals():");
        var lengthForCompare2 = new Quantity<LengthUnit>(10.0, LengthUnit.Feet);
        var weightForCompare2 = new Quantity<WeightUnit>(5.0, WeightUnit.Kilogram);
        var volumeForCompare2 = new Quantity<VolumeUnit>(2.0, VolumeUnit.Litre);

        WriteLine($"10 ft equals 5 kg? {lengthForCompare2.Equals(weightForCompare2)}");
        WriteLine($"10 ft equals 2 L? {lengthForCompare2.Equals(volumeForCompare2)}");
        WriteLine($"5 kg equals 2 L? {weightForCompare2.Equals(volumeForCompare2)}");
        WriteLine();

        // Demonstrate that different categories cannot be mixed in collections
        WriteLine("Collection Type Safety:");
        WriteLine("List<Quantity<LengthUnit>> can only contain length measurements");
        WriteLine("List<Quantity<WeightUnit>> can only contain weight measurements");
        WriteLine("List<Quantity<VolumeUnit>> can only contain volume measurements");

        WriteLine();

        
        WriteLine("TEMPERATURE MEASUREMENTS (UC14)");
        

        // Temperature equality demonstrations
        ...
        WriteLine($"celsius to celsius: {celsius1} equals {celsius2}? {celsius1.Equals(celsius2)}");
        ...
        WriteLine($"fahrenheit to fahrenheit: {fahrenheit1} equals {fahrenheit2}? {fahrenheit1.Equals(fahrenheit2)}");

        var celsius0 = new Quantity<TemperatureUnit>(0.0, TemperatureUnit.Celsius);
        var fahrenheit32 = new Quantity<TemperatureUnit>(32.0, TemperatureUnit.Fahrenheit);
        WriteLine($"0°C equals 32°F? {celsius0.Equals(fahrenheit32)}");
        ...
        WriteLine($"100°C equals 212°F? {celsius100.Equals(fahrenheit212)}");
        ...
        WriteLine($"-40°C equals -40°F? {celsiusNeg40.Equals(fahrenheitNeg40)}");

        // Temperature conversions
        WriteLine("\nTemperature Unit Conversions:");

        double celsiusToFahrenheit = new Quantity<TemperatureUnit>(100.0, TemperatureUnit.Celsius).ConvertTo(TemperatureUnit.Fahrenheit);
        WriteLine($"convert 100°C to °F: {celsiusToFahrenheit}°F");

        double fahrenheitToCelsius = new Quantity<TemperatureUnit>(32.0, TemperatureUnit.Fahrenheit).ConvertTo(TemperatureUnit.Celsius);
        WriteLine($"convert 32°F to °C: {fahrenheitToCelsius}°C");

        double celsiusNeg40ToF = new Quantity<TemperatureUnit>(-40.0, TemperatureUnit.Celsius).ConvertTo(TemperatureUnit.Fahrenheit);
        WriteLine($"convert -40°C to °F: {celsiusNeg40ToF}°F");

        double fahrenheit212ToC = new Quantity<TemperatureUnit>(212.0, TemperatureUnit.Fahrenheit).ConvertTo(TemperatureUnit.Celsius);
        WriteLine($"convert 212°F to °C: {fahrenheit212ToC}°C");

        // Unsupported arithmetic operations demonstration
        WriteLine("\nUnsupported Temperature Operations:");

        try
        {
            var temp1 = new Quantity<TemperatureUnit>(100.0, TemperatureUnit.Celsius);
            var temp2 = new Quantity<TemperatureUnit>(50.0, TemperatureUnit.Celsius);
            var result = temp1.Add(temp2);
            WriteLine($"100°C + 50°C = {result}");
        }
        catch (NotSupportedException ex)
        {
            WriteLine($"Addition not supported: {ex.Message}");
        }
        ...
        {
            WriteLine($"Subtraction not supported: {ex.Message}");
        }
        ...
        try
        {
            var temp1 = new Quantity<TemperatureUnit>(100.0, TemperatureUnit.Celsius);
            var temp2 = new Quantity<TemperatureUnit>(50.0, TemperatureUnit.Celsius);
            var result = temp1.Divide(temp2);
            WriteLine($"100°C / 50°C = {result}");
        }
        catch (NotSupportedException ex)
        {
            WriteLine($"Division not supported: {ex.Message}");
        }
        catch (DivideByZeroException ex)
        {
            WriteLine($"Division error: {ex.Message}");
        }

        // Cross-category prevention - RENAMED VARIABLES
        WriteLine("\nCross-Category Prevention (Temperature vs Others):");

        var tempForCompare1 = new Quantity<TemperatureUnit>(100.0, TemperatureUnit.Celsius);
        var lengthForCompare3 = new Quantity<LengthUnit>(100.0, LengthUnit.Feet);
        var weightForCompare3 = new Quantity<WeightUnit>(100.0, WeightUnit.Kilogram);
        var volumeForCompare3 = new Quantity<VolumeUnit>(100.0, VolumeUnit.Litre);

        WriteLine($"100°C equals 100 ft? {tempForCompare1.Equals(lengthForCompare3)}");
        WriteLine($"100°C equals 100 kg? {tempForCompare1.Equals(weightForCompare3)}");
        WriteLine($"100°C equals 100 L? {tempForCompare1.Equals(volumeForCompare3)}");
