...
namespace BusinessLayer.Services
{
    ...
    public class QuantityMeasurementService : IQuantityMeasurementService
    {
        ...
        public QuantityMeasurementEntity CompareQuantities(QuantityDTO first, QuantityDTO second)
        {
            try
            {
                _logger.LogDebug("Comparing quantities: {First} and {Second}", first, second);
                ...
                _logger.LogInformation("Comparison result: {Result}", areEqual);
        ...
        public QuantityMeasurementEntity ConvertQuantity(QuantityDTO source, string targetUnit)
        {
            try
            {
                _logger.LogDebug("Converting {Source} to {TargetUnit}", source, targetUnit);
                ...
                _repository.Save(entity);
                _logger.LogInformation("Conversion result: {Result} {Unit}", result, targetUnit);
        ...
        public QuantityMeasurementEntity AddQuantities(QuantityDTO first, QuantityDTO second, string targetUnit)
        {
            try
            {
                _logger.LogDebug("Adding quantities: {First} + {Second}", first, second);
                ...
                _repository.Save(entity);
                _logger.LogInformation("Addition result: {Result} {Unit}", result, targetUnit);
        ...
        public QuantityMeasurementEntity SubtractQuantities(QuantityDTO first, QuantityDTO second, string targetUnit)
        {
            try
            {
                _logger.LogDebug("Subtracting quantities: {First} - {Second}", first, second);
                ...
                _repository.Save(entity);
                _logger.LogInformation("Subtraction result: {Result} {Unit}", result, targetUnit);
        ...
        public QuantityMeasurementEntity DivideQuantities(QuantityDTO first, QuantityDTO second)
        {
            try
            {
                _logger.LogDebug("Dividing quantities: {First} / {Second}", first, second);
        ...
        private double ConvertFromBase(object model, double baseValue, string targetUnit)
        {
            ...
            if (model is QuantityModel<LengthUnit> length && targetUnit == GetUnitString(length.Unit))
        ...
        private QuantityMeasurementEntity HandleException(string operation, Exception ex)
        {
            string message = ex switch
            {
                ...
                DatabaseException => $"Database error: {ex.Message}",
                _ => $"Unexpected error: {ex.Message}"
            };

            _logger.LogError(ex, "Handled exception in {Operation}: {Message}", operation, message);
